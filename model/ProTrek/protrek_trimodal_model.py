...
class ProTrekTrimodalModel(AbstractModel):
    def __init__(
        ...
        load_protein_pretrained: bool = False,
        load_text_pretrained: bool = False,
    ...
    def initialize_metrics(self, stage: str) -> dict:
        ...
        if self.use_mlm_loss:
            ...
            if self.structure_config is not None:
                return_dict[f"{stage}_structure_mask_acc"] = torchmetrics.Accuracy(
                    ignore_index=-1
                )
    ...
    def forward(
        self, protein_inputs: dict, text_inputs: dict, structure_inputs: dict = None
    ):
        """
        Args:
    ...
    def loss_func(self, stage: str, outputs, labels):
        ...
        all_protein_repr = (
            self.all_gather(protein_repr).view(-1, protein_repr.shape[-1]).detach()
        )
        all_text_repr = (
        ...
        bs_labels = torch.linspace(rank * bs, rank * bs + bs - 1, bs, dtype=int).to(
            device
        )
        ...
        for k, values in pairs.items():
            for v in values:
                ...
                sim = torch.matmul(eval(f"{k}_repr"), eval(f"all_{v}_repr").T).div(
                    self.temperature
                )
        ...
        for i, (uniprot_id, subsections) in enumerate(self.uniprot2label.items()):
            ...
            for subsection, text_ids in subsections.items():
                ...
                for text_id in text_ids:
                    text2label[subsection][text_id] = text2label[subsection].get(
                        text_id, []
                    ) + [i]
